...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    api = TerneoApi(host)
    coordinator = TerneoCoordinator(hass=hass, api=api, update_interval=timedelta(seconds=scan_interval))
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    unload_ok = await hass.config_entries.async_unload_platforms(entry, ['climate','sensor','binary_sensor','calendar'])
    if unload_ok:
